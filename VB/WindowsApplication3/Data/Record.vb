...
Namespace WindowsApplication3.Data
    ...
    Public Class DataHelper
        ...
        Public Shared Function GetData(ByVal count As Integer) As BindingList(Of Record)
            Dim records As New BindingList(Of Record)()
            For i As Integer = 0 To count - 1
                records.Add(New Record() With {.ID = i, .ParentID = i Mod 5, .Text = String.Format("Text {0}", i), .Dt = If(i Mod 2 = 0, Date.Now.AddDays(i), DirectCast(Nothing, Date?)), .State = i Mod 2 = 0, .Image = SystemIcons.Information.ToBitmap()})
            Next i
            Return records
        End Function
